...
| Package | What it is |
|---|---|
| [`packages/canvas-md`](packages/canvas-md) | The core CLI and Python API: sync markdown pages, assignments, and syllabi to Canvas (LaTeX math, image uploads, PDF embeds), list resources, and view the gradebook. Defines the `canvas_md.commands` entry-point group that plugins register into. |
...
```bash
pip install -e packages/canvas-md -e packages/canvas-auto-quiz pytest
pytest packages/canvas-md/tests packages/canvas-auto-quiz/tests
```
