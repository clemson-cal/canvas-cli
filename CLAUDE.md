...
Monorepo for markdown-first Canvas LMS course-authoring tools: `packages/canvas-md` (generic core; import name `canvas_md`) and `packages/canvas-auto-quiz` (quiz-bank plugin registered via the `canvas_md.commands` entry-point group). Generic features belong in core; opinionated personal-workflow features belong in plugin packages.

## Project planning note: plan.md

`plan.md` at the repo root is the project's planning and task-tracking note. It is version-controlled here and simultaneously appears in the owner's Obsidian vault via a symlink (`~/Work/Obsidian/Projects/canvas-md.md` → this file).
